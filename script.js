...
const firebaseConfig = {
  ...
  storageBucket: "wolf-chat2.0.firebasestorage.app",
      ...
      if (typeof auth !== 'undefined' && auth && typeof auth.onAuthStateChanged === 'function') {
        auth.onAuthStateChanged(user => {
          if (user) {
            ...
            if (!storedUsername) {
              storedUsername = user.displayName || (user.email ? user.email.split('@')[0] : '');
        ...
        // If auth/firestore isn't available, just show auth screen so page still works
        document.getElementById("chat-container").style.display = "none";
        document.getElementById("auth-screen").style.display = "block";
      }
    } else { 
...
function handleAuthSubmit() {
  ...
  if (isSignUpMode) {
    ...
    if (auth && typeof auth.createUserWithEmailAndPassword === 'function') {
      ...
      alert('Auth not available in this environment.');
    }
...
function handleGoogleSignIn() {
  if (!firebase || !firebase.auth) { alert('Google SignIn not available'); return; }
...
function handleFacebookSignIn() {
  if (!firebase || !firebase.auth) { alert('Facebook SignIn not available'); return; }
...
function bypassToChat() {
  ...
  if (userPfp && (userPfp.startsWith("data:image") || userPfp.startsWith("http"))) {
  ...
  if (typeof db !== 'undefined' && db) listenToLiveMessages(currentRoomNode);
}
...
function switchChannel(channelName) {
  ...
  const el = document.getElementById("current-room-display");
  if (el) el.innerText = `Room: ${channelName}`;
  const items = document.querySelectorAll(".navigation-item");
  items.forEach(btn => btn.classList.remove("active-room"));
  if (event && event.target) event.target.classList.add("active-room");
  if (typeof db !== 'undefined' && db) listenToLiveMessages(currentRoomNode);
  toggleSidebarMenu();
}

function openSettingsModal() {
  const gallery = document.getElementById("settings-gallery-input");
  ...
  if (colorSelect) colorSelect.value = userChatColor;
  const previewCircle = document.getElementById("settings-pfp-preview-circle");
  if (previewCircle) {
    if (userPfp && (userPfp.startsWith("data:image") || userPfp.startsWith("blob:") || userPfp.startsWith("http"))) {
...
function saveSystemSettings() {
  const inputUsername = document.getElementById("settings-username-input");
  if (!inputUsername) return;
  const val = inputUsername.value.trim();
  if (val === "") return;
  currentUsername = val;
  const color = document.getElementById("settings-color-select");
  if (color) userChatColor = color.value;
...
function loadSimulatedUsers() {
  ...
  if (counter) counter.innerText = localUsers.length;
...
function handleImageSelect(event) {
  ...
  reader.onload = function(evt) {
    ...
    if (previewContent) previewContent.innerHTML = `<img src="${stagedImageUrl}" style="max-height: 60px; border-radius: 6px;" />`;
    if (previewTray) previewTray.style.display = "flex";
  ...
  if (!isRecording) {
    navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
      ...
      mediaRecorder.onstop = () => {
        ...
        const previewTray = document.getElementById("media-preview-tray");
        const previewContent = document.getElementById("preview-content");
        if (previewContent) previewContent.innerHTML = `<audio controls style="max-width: 200px;"><source src="${stagedAudioUrl}" type="audio/wav"></audio>`;
        if (previewTray) previewTray.style.display = "flex";
        stream.getTracks().forEach(track => track.stop());
      };
      mediaRecorder.start();
      isRecording = true;
      if (micBtn) { micBtn.innerText = "⏹️"; micBtn.style.background = "rgba(255, 0, 127, 0.2)"; }
    }).catch(err => alert("Microphone access denied: " + err.message));
    return false;
  } else {
    if (mediaRecorder && typeof mediaRecorder.stop === 'function') mediaRecorder.stop();
    isRecording = false;
    if (micBtn) { micBtn.innerText = "🎤"; micBtn.style.background = ""; }
    return false;
  }
}
...
function sendMessage() {
  ...
  const messageObj = {
    ...
    timestamp: (typeof firebase !== 'undefined' && firebase.firestore && firebase.firestore.Timestamp) ? firebase.firestore.Timestamp.fromDate(timestamp) : timestamp,
  ...
  if (typeof db !== 'undefined' && db) {
    ...
    if (messagesBox) {
      ...
      msgEl.innerText = `${messageObj.username}: ${messageObj.text}`;
    ...
    .onSnapshot(snapshot => {
      snapshot.docChanges().forEach(change => {
        if (change.type === "added") {
          ...
          const msgTime = msg.timestamp ? (msg.timestamp.toDate ? msg.timestamp.toDate() : new Date(msg.timestamp)) : new Date();
